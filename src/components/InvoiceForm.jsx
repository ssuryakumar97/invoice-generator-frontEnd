...
import { addInvoice, updateInvoice, updateInvoiceWithProduct } from "../redux/invoicesSlice";
...
const InvoiceForm = () => {
  ...
  const productSize = productArray.length
  ...
  const handleAddInvoice = () => {
    const productData = formData.items.map((val,ind) => {
      return { productId: productSize + 1 + ind, itemName: val.itemName, itemDescription: val.itemDescription, itemPrice: val.itemPrice, itemType: val.itemType }
    })
    const productsData = productData.filter((val,ind, arr) => {
     return arr.findIndex(o => {
        return o.itemName == val.itemName
      }) === ind
    })    
    if (isEdit) {
      ...
      dispatch(updateInvoiceWithProduct(productsData))
      ...
      dispatch(updateInvoiceWithProduct(productsData))
      alert("Invoice added successfuly 1111🥳");
    } else {
